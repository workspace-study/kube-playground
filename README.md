...
# Check both interfaces are configured
ip addr show enp0s8  # Should show {BRIDGE_NETWORK}.{CONTROL_IP_PUBLIC}
ip addr show enp0s9  # Should show {PRIVATE_NETWORK}.{CONTROL_IP_PRIVATE}
...
#### Check Node IPs
```bash
# Nodes should register with private IPs
kubectl get nodes -o wide
# InternalIP should be {PRIVATE_NETWORK}.x

# Verify API server is accessible on public IP
# Replace with your actual control IP from .env
curl -k https://<CONTROL_PUBLIC_IP>:6443
```

#### LoadBalancer Not Accessible

**See [Network-Troubleshooting.md](./Network-Troubleshooting.md) for detailed LoadBalancer troubleshooting.**
...
# Check Cilium device configuration (MUST include both enp0s8 and enp0s9)
cilium config view | grep device

# Verify L2 announcements are enabled
kubectl get cm -n kube-system cilium-config -o yaml | grep -i l2
...
# Verify VXLAN tunnels are on private network
